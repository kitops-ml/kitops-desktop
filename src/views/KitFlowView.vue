...
// Tab state — lives here because tabs are purely this view's UI concern
...
// Auto-expand the step that just started running (runs regardless of active pane)
...
<template>
  <div class="flex flex-col h-full">
    ...
    <header class="px-8 bg-elevation-02 border-b border-gray-03 h-28 shrink-0">
      <div class="flex items-center justify-between h-full gap-4">
        ...
        <div class="flex items-center gap-2 shrink-0">
          ...
          <!-- Run / Stop (only when a flow tab is active and loaded) -->
          <button
    ...
    <!-- Flow runner (active tab) -->
    <template v-else>
      <!-- Parse error state (file couldn't be read / parsed) -->
      ...
      <template v-else-if="flow">
        ...
        <div v-if="activePane === 'raw'" class="flex-1 overflow-y-auto">
          ...
          <!-- Line-by-line view with error highlights -->

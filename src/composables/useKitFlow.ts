...
export interface KitFlowVarDef {
  name: string
  default: string | undefined // undefined = required
  isDerived: boolean          // default is an expression referencing other vars
}
...
export function interpolate(value: string, vars: Record<string, string>): string {
  return value.replace(/\$\{([^}]+)\}/g, (match, expr) => {
    ...
    return filters.reduce((v: string, f: string) => applyFilter(v, f), vars[name])
  })
}
...
// ── Module-level singleton state (persists for the entire app session) ─────────

const importedFlows = ref<ImportedFlow[]>(readLibrary())
const running = ref(false)

// Active session reactive state — mirrors the currently selected tab's session
...
// Re-derive expression-based var defaults whenever any var changes,
// but only for vars the user hasn't manually edited.
watch(userVars, () => {
  if (!flow.value) {
    return
  }
...
export function initUserVars(): void {
  ...
  dirtyVars.value = new Set()
  lastDerived.value = {}
  // Two-pass: non-derived first so derived expressions can reference them
...
export async function loadFlow(flowPath: string): Promise<boolean> {
  ...
  // Force raw view when there are validation errors so the user sees highlights
...
async function addToLibrary(flowPath: string, name?: string, description?: string): Promise<void> {
  if (!importedFlows.value.some(f => f.path === flowPath)) {
    ...
    // If not provided, read from the YAML
    if (name === undefined) {
      const fileResult = await window.kitops.fs.readFile(flowPath)
      if (fileResult.success) {
        try {
        ...
        } catch { /* ignore parse errors; we'll show them when the tab loads */ }
      }
    }

    const entry: ImportedFlow = {
      path: flowPath,
      name: resolvedName,
    ...
    importedFlows.value.unshift(entry)
...
async function run(flowPath: string): Promise<void> {
  ...
  // Pass non-derived vars by name; the CLI will compute derived vars itself.
  ...
  window.kitops.kitflow.onEvent((event: KitflowEvent) => {
    switch (event.type) {
      ...
      case 'error': {
        // Unexpected stderr output from the process (e.g. a Node.js runtime error).
        // Surface it on whichever step is currently running so it isn't silently dropped.
  ...
  try {
    ...
    // Step-level errors are already reflected via step-done events.
  } finally {
